...
def group_by_book(directory):
    ...
    output_dir = os.path.join(directory, "grouped_books")
    os.makedirs(output_dir="D:\\Projects\\oper\\grouped", exist_ok=True)
    ...
    for book_number, book_entries in book_groups.items():
        output_filepath = os.path.join(output_dir, f"Book_{book_number:02d}.json")
